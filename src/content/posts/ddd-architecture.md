...
战略设计通过领域、子域、限界上下文和统一语言，为 DDD 大厦立起栋梁；战术设计中的实体、值对象、聚合、资源库、领域服务和领域事件，则提供构建领域模型的砖瓦。但有了栋梁和砖瓦，还不等于建成一座能够长期演进的大厦：系统仍需明确哪些组件承担用例编排，哪些组件实现业务规则，哪些组件处理数据库、消息和网络等技术细节，以及依赖应该朝什么方向流动。

架构决定这些栋梁和砖瓦如何组织、连接与协作。它不直接决定每一块砖瓦的内容，却通过职责划分和依赖约束，保护最重要的部分不被外界变化轻易破坏。
...
目录名称、框架注解和六边形图都只是结构的外在表现。真正的架构是清晰的职责、稳定的边界和朝向核心的依赖方向。只有这些约束真实存在，领域模型才不会在一次次技术更换和需求迭代中逐渐被侵蚀。
