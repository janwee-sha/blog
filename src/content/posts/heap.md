...
堆（Heap）是一种以完全二叉树组织数据的结构，其核心约束是父节点与子节点之间始终满足特定的大小关系。根节点通常称为堆顶；在最大堆中，堆顶保存最大值；在最小堆中，堆顶保存最小值。

堆的常见用途包括：

-   **堆排序**：将待排序元素组织成堆，反复取出堆顶并调整剩余元素，从而得到有序序列。
-   **事件模拟**：按照事件的预定发生时间维护最小堆，以便快速选择下一个需要处理的事件。
-   **优先队列**：根据任务优先级维护元素，使调度程序能够高效取出当前最重要的任务。
-   **Dijkstra 算法**：使用二叉堆或斐波那契堆实现优先队列，降低选取当前最短路径候选节点的成本。
...
1.  [堆积](https://zh.wikipedia.org/wiki/%E5%A0%86%E7%A9%8D)（维基百科，自由的百科全书）
