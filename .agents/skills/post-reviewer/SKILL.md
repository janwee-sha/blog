...
6. 检查必检清单的所有类别，包括从 `cicd-pipeline-with-github-actions-ghcr-docker.md` 提取的标准开篇引文、编号章节层级与引用列表结构，以及因读者环境而异的示例值是否附有明确的替换提示。将这些约定视为仓库发布规则，而非可选示例；首章只需在 `## 01.` 后使用有意义的标题，不强制使用 `引言`。在此阶段验证本地链接和去重后的外部链接。检查期间不得编辑。
