...
这一次，我们创建一个核心线程数和最大线程数都为 1、工作队列初始容量为 2 的线程池，并指定饱和策略为丢弃最老任务。随后向线程池提交三个任务，分别向本地队列插入整数 1、2、3。提交第三个任务时，线程池达到饱和状态，最老的任务（即插入 1 的任务）被丢弃，新任务进入工作队列，因此本地队列最终得到 2 和 3。

## 引用

1.  [ThreadPoolExecutor（Java Platform SE 8）](https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/ThreadPoolExecutor.html)
