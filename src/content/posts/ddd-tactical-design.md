...
战略设计帮助我们识别业务问题、划定模型边界，为 DDD 大厦立起栋梁；战术设计则帮助我们在这些边界内部构建模型。实体、值对象、聚合、资源库、领域服务和领域事件，是 DDD 中最常见的一组战术设计模式。
...
一堆精心命名的类并不会自动构成领域模型。只有当这些砖瓦依托清晰的模型边界，按照业务规则组合起来，代码才真正开始表达业务。
