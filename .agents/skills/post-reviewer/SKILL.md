---
name: post-reviewer
description: 交互式审阅并修改本博客站点仓库中 src/content/posts 下的一篇或全部 Markdown 文章。当用户要求 review/审阅、format/格式化、proofread/校对、lint/检查、normalize/规范化或 validate/验证博客文章时使用；覆盖 Markdown 与 Astro frontmatter、文章路径与 slug、中英文间距、错别字与语法、一致性、图片与链接以及 publication readiness/发布就绪度。始终先检查并展示修复建议和需判断的问题，等待用户反馈后仅应用获批改动，再执行验证。接受 scope=all，或 slug、文件名、仓库相对路径。
---
...
- `scope=<slug|filename|repo-relative-path>`：只选择一篇文章。如：`ai/langextract`、`langextract.md` 或 `src/content/posts/ai/langextract.md`。
...
2. 若有同时支持固定选项和内置自由输入的交互工具，显示以下用户可见选项：
   - 辅助脚本返回的完整文章 `scope`，例如 `ai/langextract`；推荐此项，并用标题与发布日期说明；
   - `所有文章`，映射为 `scope=all`；
...
5. 完整读取每篇选中的文章。判断术语、标题结构、编号、标点、一致性，以及分类与文章 slug 的英文语法和语义时，检查整个文件；不得根据片段推断。
...
- **Needs user review（需用户复核）**：措辞或语气调整、存在多个有效修法、`title`/`description`/frontmatter 改动、文件重命名或移动、slug 改动、标题锚点改动、缺失或不准确的开篇引文、缺失的书目信息、事实或 API 更新、可运行代码改动、链接替换、发布日期，以及任何可能改变语义的内容。展示推荐选项与简短的替代方案或取舍。
...
3. 对同一 `scope` 重新运行 `audit_posts.py`；路径已获批变更时改用新 `scope`。仅当剩余诊断项属于已批准类别时才修复。若出现新的需判断问题，先返回 Phase 2。
...
- 保持言简意赅。检查单篇文章以及所选范围内系列文章的语义重复；相同或高度相似的定义、论点、比喻、过渡和总结若未增加新信息，只保留在最能推进论述的一处。标题中的意象无需在各篇正文反复解释，跨篇回顾只保留理解当前内容所必需的信息。
...
- 将文件重命名、文章移动和父目录重命名都视为 route slug 与 URL 变更。展示新旧 slug、链接与 SEO 影响并等待明确批准；父目录变更不得隐式波及 scope 外文章。获批后全仓只读搜索旧 URL，在获批 scope 内更新链接；scope 外改动需另行批准，否则添加仓库支持的 redirect，最后验证新 route。
