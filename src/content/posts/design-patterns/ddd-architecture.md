...
战略设计明确业务重点和模型边界，战术设计通过实体、值对象、聚合、资源库、领域服务和领域事件在边界内部构建领域模型。但只有边界和模型元素，还不足以形成一套能够长期演进的系统：系统仍需明确哪些组件承担用例编排，哪些组件实现业务规则，哪些组件处理数据库、消息和网络等技术细节，以及依赖应该朝什么方向流动。

架构决定各部分如何组织、连接与协作。它不直接决定每个模型元素的内容，却通过职责划分和依赖约束，保护最重要的部分不被外界变化轻易破坏。
