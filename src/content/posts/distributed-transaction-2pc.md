...
常见的分布式事务有 2PC、TCC 和 SAGA。下文先只讨论 2PC。
...
## 03. 使用 2PC 实现一个简单的分布式事务

下面给出一个 Java 代码示例：它基于 2PC 模型，使用 PostgreSQL 原生的 `PREPARE TRANSACTION` 模拟上述转账场景：
...
> [!NOTE]
> 将代码中的数据库连接地址、数据库用户名和密码替换为你的实际配置。
